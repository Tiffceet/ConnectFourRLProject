...
def custom_flatten(board, flip_players=False):
    flat = board.flatten()
    if flip_players:
...
def run_game(algo_model, plot_x, plot_y, is_ai_first=False):
    game_count = 3
    win = 0
    # is_ai_first = True
    for count in range(game_count):
        ...
        while not game_over:
            ...
            if turn == PLAYER_TURN:
                col = algo_model.predict(custom_flatten(board, is_ai_first))[0]
...
run_game(ars_model, ars_plot_x, ars_plot_y)
plt.plot(ars_plot_x, ars_plot_y, color='r', label='ARS as first player')

run_game(ars_model, minimax_plot_x, minimax_plot_y, True)
plt.plot(minimax_plot_x, minimax_plot_y, color='b',
         label='Minimax as first player')
